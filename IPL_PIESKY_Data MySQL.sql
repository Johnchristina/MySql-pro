...
select * from `ipl_bidding_details` where bidder_id=102;

-- The corresponding score in table  IPL_Bidder_Points for bidder_id 102 is 0;

-- Q1: The percentage of wins of each bidder in the order of highest to lowest percentage.
-- Solution

select * from
(select bidder_id, sum(win_status)/count(win_status) as percentage_of_wins_to_bids
from
(select bidder_id, bid_status, if(bid_status='won',1,0) Win_status
from ipl_bidding_details)DT1
group by bidder_id)DT2
order by percentage_of_wins_to_bids desc;


-- Q2: Display the number of matches conducted at each stadium with stadium name, city from the database
-- Solution

select DT.stadium_id,ST.stadium_name,ST.city,DT.matches_conducted 
from
(select stadium_id ,count(*) as matches_conducted
from ipl_match_schedule
where status='completed'
group by stadium_id)DT inner join
ipl_stadium ST on DT.stadium_id= ST.stadium_id;

-- Q3:	In a given stadium, what is the percentage of wins by a team which has won the toss?
-- Solution

select stadium_id, sum(toss_match_win)/count(toss_match_win) as percentage_of_wins_who_won_toss
from
(select m.match_id,ms.stadium_id, if(toss_winner=match_winner,1,0) as Toss_match_win
from ipl_match M
inner join ipl_match_schedule MS
on M.match_id=MS.match_id
where MS.status='completed')DT
group by stadium_id;

-- Q4.	Show the total bids along with bid team and team name.
-- Solution

select bid_team,team_name, total_bids
from (select bid_team, count(*) as total_bids
 from `ipl_bidding_details`
group by bid_team)DT inner join ipl_team 
on bid_team=team_id;
...
else 'others' end winner_team_id
from ipl_match;
...
select TS.team_id, team_name, `matches_played`, `matches_won`,`matches_lost` from  
...
group by team_id)TS inner join ipl_team T on TS.team_id= T.team_id;
...
from ipl_player 
where player_id in (select player_id, player_role
from ipl_team_players
where  ((player_role='bowler') or (player_role='all-rounder'))and team_id=(select team_id from ipl_team where team_name='mumbai indians'));
...
from ipl_team_players
group by team_id, player_role
having  player_role='all-rounder' and count(*)>4
order by count(*) desc;